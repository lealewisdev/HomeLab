...
Infrastucture as code for a NixOS based server hosting 70+ services in one Docker Compose project, a signed and scanned image pipeline, and automated deployment with rollback. This repository only contains select files and does not reflect the directory structure of my server. Secrets, environment files and machine-specific IDs are redacted.
...
- **Failover** Availability relies on restart policies, health checks and monitoring. If I had a spare machine I would migrate from Docker Compose to K3's
